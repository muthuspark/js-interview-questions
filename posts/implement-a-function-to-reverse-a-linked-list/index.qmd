...
3. **Storing the Next Node:**  `next = current.next;` saves a reference to the next node in the list before we modify `current.next`. This is crucial to avoid losing the rest of the list.
